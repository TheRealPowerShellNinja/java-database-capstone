...
public class AppointmentService {
    ...
    public ResponseEntity<Map<String, Object>> updateAppointment(Appointment appointment, String token) {
        ...
        try {
            ...
            if (!service.validateAppointment(doctor.getId(), requestedSlot)) {
    ...
    @Transactional
    public ResponseEntity<Map<String, Object>> getAppointments(String token, String date, String patientName) {
        Map<String, Object> response = new HashMap<>();
        ...
        try {
            ...
            LocalDateTime end = localDate.plusDays(1).atStartOfDay().minusNanos(1);
            ...
            List<AppointmentDTO> dtoList = appointments.stream().map(a ->
                    new AppointmentDTO(
                            a.getId(),
            ...
            response.put("message", "Error loading appointments");
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(response);
        }
    }
